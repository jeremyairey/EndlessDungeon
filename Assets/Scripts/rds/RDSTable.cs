			...
			get
			{
				...
				if (realdropcnt > 0)
				{
					for (int dropcount = 0; dropcount < realdropcnt; dropcount++)
					{
						...
						//Get sum for next entry here.
						//JWA
					//	foreach (double value in dropables)
					//	{
					//		outValue += value;
					//		
					//	}
						
				//		double hitvalue = RDSRandom.GetDoubleValue(outValue);
					...
					double hitvalue = RDSRandom.GetDoubleValue(dropables.Select(e => e.rdsProbability).Sum());	
